...
`tools/list` at `https://gateway.pipeworx.io/entreprises-fr/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/entreprises_fr_search \
  -H 'Content-Type: application/json' \
  -d '{"query":"Acme SARL Paris"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/entreprises_fr_search`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
...
```json
{
  "mcpServers": {
    "entreprises-fr": {
      "command": "npx",
      "args": ["-y", "@pipeworx/mcp-entreprises-fr"]
    }
  }
}
```
...
```bash
npx -y @pipeworx/mcp-entreprises-fr
```
